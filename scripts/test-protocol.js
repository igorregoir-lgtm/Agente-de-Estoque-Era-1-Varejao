// Varejão Era 1 — Protocol Verification Script
// Run using `node scripts/test-protocol.js`

import { runInventoryProtocol } from '../src/utils/protocol.js';
import { sampleSkus } from '../src/utils/seedData.js';

import fs from 'fs';
import path from 'path';

// Resolve sample data and protocol paths relative to project root
const skusList = [
...
console.log('Running Varejão Era 1 Protocol Verification Tests...');

try {
  const result = runInventoryProtocol(skusList);

  // Assertions
  const junta = result.skus.find(x => x.sku === 'JUNT-TM-001');
  const pastilha = result.skus.find(x => x.sku === 'PAST-FR-001');
  const sensor = result.skus.find(x => x.sku === 'SENS-OX-001');

  console.log(`\n- JUNT-TM-001: Stock=${junta.estoque}, Alert=${junta.nivelAlerta}`);
  console.log(`- PAST-FR-001: Stock=${pastilha.estoque}, Alert=${pastilha.nivelAlerta}`);
  console.log(`- SENS-OX-001: Stock=${sensor.estoque}, Alert=${sensor.nivelAlerta}`);

  let errors = 0;

  if (junta.nivelAlerta !== '🔴 CRÍTICO') {
    console.error('FAIL: JUNT-TM-001 must trigger 🔴 CRÍTICO alert.');
    errors++;
  } else {
    console.log('PASS: JUNT-TM-001 alert is correct.');
  }

  if (pastilha.nivelAlerta !== '🟡 ALTO') {
    console.error('FAIL: PAST-FR-001 must trigger 🟡 ALTO alert.');
    errors++;
  } else {
    console.log('PASS: PAST-FR-001 alert is correct.');
  }

  if (sensor.nivelAlerta !== '🟡 ALTO') {
    console.error('FAIL: SENS-OX-001 must trigger 🟡 ALTO alert.');
    errors++;
  } else {
    console.log('PASS: SENS-OX-001 alert is correct.');
  }

  // Check metrics
  console.log(`\nMetrics Summary:`);
  console.log(`- Total Capital Imobilizado: R$ ${result.summary.totalCapitalImobilizado.toFixed(2)}`);
  console.log(`- Capital em Giro Lento: R$ ${result.summary.capitalGiroLento.toFixed(2)}`);
  console.log(`- Receita em Risco: R$ ${result.summary.totalReceitaEmRisco.toFixed(2)}`);
  console.log(`- Total Sugerido de Reposição: R$ ${result.summary.totalReplenishmentCost.toFixed(2)}`);

  if (result.summary.totalCapitalImobilizado <= 0) {
    console.error('FAIL: Total capital imobilizado must be positive.');
    errors++;
  }
  if (result.summary.totalReceitaEmRisco <= 0) {
    console.error('FAIL: Total revenue at risk must be positive.');
    errors++;
  }

  if (errors > 0) {
    console.error(`\nTest completed with ${errors} error(s).`);
    process.exit(1);
  } else {
    console.log('\nALL TESTS PASSED SUCCESSFULLY! The protocol logic matches expectations.');
    process.exit(0);
  }
} catch (err) {
